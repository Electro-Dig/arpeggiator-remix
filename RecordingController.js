...
export class RecordingController extends EventTarget {
  constructor({
    ...
    this.getPhotoCaptureSource = getPhotoCaptureSource;
    this.getPhotoOverlays = getPhotoOverlays;
    this.capturePhoto = capturePhoto;
    this.requestFrame = requestFrame;
    this.getPerformanceMetadata = getPerformanceMetadata;
    ...
    this.currentPerformanceMetadata = Object.freeze({});
  ...
  collectElements() {
    ...
    return {
      ...
      shareQr: byId('recording-share-qr'),
      frameFallback: byId('recording-share-frame-fallback'),
      ...
      cover: byId('recording-photo-preview'),
      gestureStatus: byId('recording-gesture-status'),
    };
  }
  ...
  bind() {
    ...
    this.elements.confirm?.addEventListener('click', () => {
      if (this.state.phase === 'review') this.dispatch({ type: 'UPLOAD_REQUEST' });
    });
    ...
    this.elements.cancel?.addEventListener('click', () => {
      if (this.state.phase === 'countdown') {
        ...
        this.dispatch({ type: 'STOP_REQUEST' });
      } else if (['review', 'shared'].includes(this.state.phase)) {
  ...
  dispatch(event) {
    ...
    if (normalizedEvent.type === 'START_REQUEST') this.clearCover();
    ...
    if (normalizedEvent.type === 'UPLOAD_REQUEST') {
  ...
  beginRecording() {
    ...
    try {
      ...
      this.closeDialog();
      this.currentPerformanceMetadata = Object.freeze({ ...this.getPerformanceMetadata() });
      this.clearCover();
      const captureGeneration = this.coverGeneration;
      this.requestFrame(() => this.captureFirstFrame(captureGeneration));
  ...
  async captureFirstFrame(generation) {
    try {
      const source = this.getVideoSource?.();
      const targetCanvas = this.elements.cover;
      if (!targetCanvas || generation !== this.coverGeneration) return null;
      const captureRequest = Object.freeze({
        root: this.root,
        cameraSource: source,
        targetCanvas,
      });
      const [captureSource, overlays] = await Promise.all([
        this.getPhotoCaptureSource?.(captureRequest),
        this.getPhotoOverlays?.(captureRequest),
      ]);
      if (generation !== this.coverGeneration) return null;
      if (!source && !captureSource) return null;
      await this.capturePhoto(source, targetCanvas, {
        captureSource: captureSource || null,
        overlays: Array.isArray(overlays) ? overlays : [],
      });
      if (generation !== this.coverGeneration) return null;
      this.currentCover = targetCanvas;
      return targetCanvas;
    } catch {
      if (generation === this.coverGeneration) this.currentCover = null;
      return null;
    }
  }
  ...
  clearCover() {
    ...
    this.currentCover = null;
    const canvas = this.elements.cover;
  ...
  async uploadCurrentTake() {
    ...
    try {
      ...
      await this.renderQr(this.elements.qr, result.shareUrl, {
        qrPreview: this.elements.shareQr,
        photo: this.currentCover,
        checkinNumber: result.checkinNumber,
        durationMs: this.takeDurationMs,
        metadata: this.currentPerformanceMetadata,
      });
  ...
  clearShareResult() {
    ...
    if (this.elements.qr) this.elements.qr.hidden = true;
    if (this.elements.cover) this.elements.cover.hidden = true;
    if (this.elements.frameFallback) this.elements.frameFallback.hidden = true;
  ...
  async showShareResult() {
    ...
    const hasFrame = Boolean(this.currentCover && this.currentCover === this.elements.cover);
    if (this.elements.cover) this.elements.cover.hidden = !hasFrame;
    if (this.elements.frameFallback) this.elements.frameFallback.hidden = hasFrame;
    if (this.elements.qr) this.elements.qr.hidden = true;
    if (this.elements.shareQr) this.elements.shareQr.hidden = false;
    if (this.elements.shareLink) {
  ...
  setGestureFeedback({ armed = true, intent = 'neutral', progress = 0, enabled = true } = {}) {
    const status = this.elements.gestureStatus;
    if (!status) return;
    ...
    if (!enabled) {
      ...
      status.textContent = '录音手势已关闭，请使用下方按钮';
    } else if (!armed) {
      status.dataset.state = 'rearming';
      status.textContent = '先放下双手，0.5 秒后再点赞确认';
    } else if (intent === 'both-up') {
      const percent = Math.round(Math.max(0, Math.min(1, Number(progress) || 0)) * 100);
      status.dataset.state = 'holding';
      status.textContent = `保持双手点赞 ${percent}%`;
    } else {
      status.dataset.state = 'ready';
      status.textContent = '双手点赞并保持 0.8 秒确认';
    }
  }
  ...
  render() {
    ...
    if (this.elements.primary) {
      this.elements.primary.disabled = [
        'stopping', 'review', 'uploading', 'error',
      ].includes(phase);
    ...
    if (this.elements.confirm) {
      this.elements.confirm.hidden = !hasTake || phase !== 'review';
      this.elements.confirm.textContent = '确认并生成海报';
    }
    if (this.elements.rerecord) {
      this.elements.rerecord.hidden = phase !== 'review';
      this.elements.rerecord.textContent = '重新录制';
    }
    if (this.elements.download) {
      this.elements.download.hidden = !hasTake || !['review', 'shared'].includes(phase);
    }
    if (this.elements.cancel) {
      this.elements.cancel.hidden = ![
        'countdown', 'recording', 'review', 'shared',
      ].includes(phase);
    ...
    const showSharedFrame = phase === 'shared'
      && Boolean(this.shareResult)
      && this.currentCover === this.elements.cover;
    if (this.elements.cover) this.elements.cover.hidden = !showSharedFrame;
    if (this.elements.frameFallback) this.elements.frameFallback.hidden = phase !== 'shared' || !this.shareResult || showSharedFrame;
    if (this.elements.qr) this.elements.qr.hidden = true;
  ...
  destroy() {
    this.clearCountdownTimer();
    this.clearStopTimer();
    if (this.elements.gestureStatus && phase !== 'review') {
      this.elements.gestureStatus.hidden = true;
    }
    this.clearElapsedTimer();
    if (this.recorder?.state === 'recording') this.recorder.stop();
    this.destroyPreviewUrl();
