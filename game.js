...
export var Game = /*#__PURE__*/ function() {
    ...
    _create_class(Game, [
        ...
        },
        {
            ...
            value: function _startGame() {
                ...
                this.musicManager.start().then(function() {
                    ...
                    // 设置全局变量供预设选择器使用
                    _this.setupGlobalVariables();
                    
                    // 初始化自定义编辑器
                    _this.initCustomEditor();
        ...
        {
            key: "setupGlobalVariables",
            value: function setupGlobalVariables() {
                // 暴露管理器给自定义编辑器使用
                window.musicManager = this.musicManager;
                window.drumManager = drumManager;
                window.drumPresets = drumManager.drumPresets;
                window.game = this;
            }
        },
        {
            key: "initCustomEditor",
            value: function initCustomEditor() {
                // 延迟初始化编辑器，确保DOM已加载
                setTimeout(() => {
                    this.customEditor = new CustomEditor();
                    window.customEditor = this.customEditor;
